...
class EncSwiftclient:
    ...
    def __init__(self, auth_token, project_id):
        ...
        self.swift_conn = client.Connection(preauthtoken=auth_token,preauthurl=storage_url, auth_version='2.0')
    ...
    def getUserID(self,username):
        ...
        param: username
        """
        ret_id = filter(lambda x: x.name == username, self.kc_conn.users.list())[0].id
        return ret_id
    ...
    def get_container (self, container, marker, delimiter, prefix):
    ...
    def delete_object(self, container, obj):
        if obj[-1] == '/':
            meta, objects = self.get_container(container, delimiter='/', prefix=obj)
            if len(objects) > 1:
